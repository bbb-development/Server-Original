...
async function loadAxiosInstance() {
  try {
    ...
    log('✅ Axios instance (re)loaded','SUCCESS');
...
app.use(rateLimit({windowMs:10_000,max:30})); // simple burst limiter
...
app.post('/request', async (req,res)=>{
  if (!axiosInstance) return res.status(500).json({error:'not_loaded'});
  const {method='GET',url,headers={},params={},data=null,timeout} = req.body || {};
